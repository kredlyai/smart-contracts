 ...
 * @author Kredly
 * @notice NativeTokenGateway contract facilitates interactions with a leToken market for native tokens (Native or wNativeToken)
 */
contract NativeTokenGateway is INativeTokenGateway, Ownable2Step, ReentrancyGuard {
     ...
     * @param redeemTokens The amount of tokens to be redeemed. This can refer to either the underlying tokens directly or their equivalent leTokens
     * @param isUnderlying A boolean flag indicating whether the redemption is for underlying tokens directly (`true`) or for their equivalent leTokens (`false`).
     * @custom:error ZeroValueNotAllowed is thrown if redeemTokens is zero
     * @custom:event TokensRedeemedAndUnwrapped is emitted when assets are redeemed from a market and unwrapped
     */
