 ...
 *
 * The directory of pools is managed through two mappings: `_poolByComptroller` which is a hashmap with the comptroller address as the key and `KredlyPool` as
...
contract PoolRegistry is Ownable2StepUpgradeable, AccessControlledV8, PoolRegistryInterface {
     ...
     */
    mapping(address => kredlyPoolMetaData) public metadata;
    ...
    /**
     * @dev Maps comptroller address to Kredly pool Index.
     */
    mapping(address => KredlyPool) private _poolByComptroller;
    ...
    /**
     * @notice Emitted when a new Kredly pool is added to the directory.
     */
    event PoolRegistered(address indexed comptroller, KredlyPool pool);
    ...
    event PoolMetadataUpdated(
        ...
        kredlyPoolMetaData oldMetadata,
        kredlyPoolMetaData newMetadata
    );
    ...
    /**
     * @notice Adds a new Kredly pool to the directory
     ...
     * @return index The index of the registered Kredly pool
    ...
    /**
     * @notice Modify existing Kredly pool name
    ...
    function setPoolName(address comptroller, string calldata name) external {
        ...
        KredlyPool storage pool = _poolByComptroller[comptroller];
     ...
     */
    function updatePoolMetadata(address comptroller, kredlyPoolMetaData calldata metadata_) external {
        _checkAccessAllowed("updatePoolMetadata(address,kredlyPoolMetaData)");
        kredlyPoolMetaData memory oldMetadata = metadata[comptroller];
        metadata[comptroller] = metadata_;
        emit PoolMetadataUpdated(comptroller, oldMetadata, metadata_);
    }

    /**
     * @notice Returns arrays of all Kredly pools' data
     * @dev This function is not designed to be called in a transaction: it is too gas-intensive
     * @return A list of all pools within PoolRegistry, with details for each pool
     */
    function getAllPools() external view override returns (KredlyPool[] memory) {
        uint256 numberOfPools_ = _numberOfPools; // storage load to save gas
        KredlyPool[] memory _pools = new KredlyPool[](numberOfPools_);
     ...
     * @return  Returns Kredly pool
     */
    function getPoolByComptroller(address comptroller) external view override returns (KredlyPool memory) {
        return _poolByComptroller[comptroller];
    }

    /**
     * @param comptroller comptroller of Kredly pool
     * @return Returns Metadata of Kredly pool
     */
    function getkredlyPoolMetadata(address comptroller) external view override returns (kredlyPoolMetaData memory) {
    ...
    /**
     * @dev Adds a new Kredly pool to the directory (without checking msg.sender).
     * @param name The name of the pool
     * @param comptroller The pool's Comptroller proxy contract address
     * @return The index of the registered Kredly pool
     */
    function _registerPool(string calldata name, address comptroller) internal returns (uint256) {
        KredlyPool storage storedPool = _poolByComptroller[comptroller];
        ...
        KredlyPool memory pool = KredlyPool(name, msg.sender, comptroller, block.number, block.timestamp);
