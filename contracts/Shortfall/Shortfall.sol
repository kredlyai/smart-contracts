    ...
    TokenDebtTracker
{
    ...
    struct Auction {
        ...
        uint256 startBidBps;
        mapping(LeToken => uint256) marketDebt;
        mapping(LeToken => uint256) bidAmount;
    }
    ...
    event MinimumPoolBadDebtUpdated(
        uint256 oldMinimumPoolBadDebt,
        uint256 newMinimumPoolBadDebt
    );

    /// @notice Emitted when wait for first bidder block count is updated
    event WaitForFirstBidderUpdated(
        uint256 oldWaitForFirstBidder,
        uint256 newWaitForFirstBidder
    );

    /// @notice Emitted when next bidder block limit is updated
    event NextBidderBlockLimitUpdated(
        uint256 oldNextBidderBlockLimit,
        uint256 newNextBidderBlockLimit
    );
    ...
    function placeBid(
        ...
        for (uint256 i; i < marketsCount; ++i) {
            LeToken leToken = LeToken(address(auction.markets[i]));
            IERC20Upgradeable erc20 = IERC20Upgradeable(
                address(leToken.underlying())
            );

            if (auction.highestBidder != address(0)) {
                _transferOutOrTrackDebt(
                    erc20,
                    auction.highestBidder,
                    auction.bidAmount[auction.markets[i]]
                );
            }
    ...
    function closeAuction(address comptroller) external nonReentrant {
        ...
        for (uint256 i; i < marketsCount; ++i) {
            ...
            uint256 balanceBefore = erc20.balanceOf(
        ...
        _transferOutOrTrackDebt(
            IERC20Upgradeable(convertibleBaseAsset),
            auction.highestBidder,
            riskFundBidAmount
        );
    ...
    function startAuction(
        ...
        _startAuction(comptroller, priceUpdateData);
    }
     ...
     */
    function updateNextBidderBlockLimit(
        uint256 _nextBidderBlockLimit
    ) external {
        _checkAccessAllowed("updateNextBidderBlockLimit(uint256)");
        require(
            _nextBidderBlockLimit != 0,
            "_nextBidderBlockLimit must not be 0"
        );
        uint256 oldNextBidderBlockLimit = nextBidderBlockLimit;
        nextBidderBlockLimit = _nextBidderBlockLimit;
        emit NextBidderBlockLimitUpdated(
            oldNextBidderBlockLimit,
    ...
    function updateMinimumPoolBadDebt(uint256 _minimumPoolBadDebt) external {
        ...
        emit MinimumPoolBadDebtUpdated(
            oldMinimumPoolBadDebt,
            _minimumPoolBadDebt
        );
    }
    ...
    function updateWaitForFirstBidder(uint256 _waitForFirstBidder) external {
        ...
        emit WaitForFirstBidderUpdated(
            oldWaitForFirstBidder,
            _waitForFirstBidder
        );
    }
    ...
    function _startAuction(
        ...
        PoolRegistryInterface.KredlyPool memory pool = PoolRegistry(
            poolRegistry
        ...
        uint256 marketsCount = auction.markets.length;
        for (uint256 i; i < marketsCount; ++i) {
            LeToken leToken = auction.markets[i];
            auction.marketDebt[leToken] = 0;
        }

        delete auction.markets;

        LeToken[] memory leTokens = _getAllMarkets(comptroller);
        marketsCount = leTokens.length;
        ResilientOracleInterface priceOracle = _getPriceOracle(comptroller);
        uint256 poolBadDebt;

        uint256[] memory marketsDebt = new uint256[](marketsCount);
        auction.markets = new LeToken[](marketsCount);

        for (uint256 i; i < marketsCount; ++i) {
            uint256 marketBadDebt = leTokens[i].badDebt();

            priceOracle.updatePrice(address(leTokens[i]), priceUpdateData);
            uint256 usdValue = (priceOracle.getUnderlyingPrice(
                address(leTokens[i])
            ) * marketBadDebt) / EXP_SCALE;

            poolBadDebt = poolBadDebt + usdValue;
            auction.markets[i] = leTokens[i];
            auction.marketDebt[leTokens[i]] = marketBadDebt;
            marketsDebt[i] = marketBadDebt;
        }

        require(poolBadDebt >= minimumPoolBadDebt, "pool bad debt is too low");

        priceOracle.updateAssetPrice(
            riskFund.convertibleBaseAsset(),
            priceUpdateData
        );
        uint256 riskFundBalance = (priceOracle.getPrice(
            riskFund.convertibleBaseAsset()
        ) * riskFund.getPoolsBaseAssetReserves(comptroller)) / EXP_SCALE;
        uint256 remainingRiskFundBalance = riskFundBalance;
        uint256 badDebtPlusIncentive = poolBadDebt +
            ((poolBadDebt * incentiveBps) / MAX_BPS);
    ...
    function _getPriceOracle(
        address comptroller
    ) internal view returns (ResilientOracleInterface) {
        return
            ResilientOracleInterface(
                ComptrollerViewInterface(comptroller).oracle()
            );
    }
     ...
     * @param comptroller Address of the pool's comptroller
     * @return markets The pool's markets as LeToken array
     */
    function _getAllMarkets(
        address comptroller
    ) internal view returns (LeToken[] memory) {
