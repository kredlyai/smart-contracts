...
import {IProtocolShareReserve} from "./protocal-reserve-interfaces/IProtocolShareReserve.sol";
import {IComptroller} from "./protocal-reserve-interfaces/IComptroller.sol";
import {IPoolRegistry} from "./interfaces/IPoolRegistry.sol";
import {ILeToken} from "./interfaces/ILeToken.sol";
import {IIncomeDestination} from "./protocal-reserve-interfaces/IIncomeDestination.sol";
    ...
    IProtocolShareReserve
{
    ...
    mapping(address => mapping(address => mapping(Schema => uint256)))
        public assetsReserves;
    ...
    event DistributionConfigRemoved(
        address indexed destination,
        uint16 percentage,
        Schema schema
    );
        ...
        for (uint256 i = 0; i < configs.length; ) {
            ...
            for (uint256 j = 0; j < distributionTargetsLength; ) {
                ...
                if (
                    ...
                    config.destination == _config.destination
                ) {
    ...
    function removeDistributionConfig(
        ...
        if (found) {
            ...
            distributionTargets[distributionIndex] = distributionTargets[
    ...
    function getUnreleasedFunds(
        ...
        for (uint256 i = 0; i < distributionTargetsLength; ) {
            ...
            if (
                _config.schema == schema && _config.destination == destination
            ) {
    ...
    function updateAssetsState(
        ...
        if (
            IPoolRegistry(poolRegistry).getLeTokenForAsset(
                comptroller,
                asset
            ) ==
            address(0)
        ...
        uint256 currentBalance = IERC20Upgradeable(asset).balanceOf(
            address(this)
        );
    ...
    function _releaseFund(address comptroller, address asset) internal {
        ...
        for (uint256 schemaValue; schemaValue < totalSchemas; ) {
            schemaBalances[schemaValue] = assetsReserves[comptroller][asset][
                Schema(schemaValue)
            ];
        ...
        for (uint256 i = 0; i < distributionTargets.length; ) {
            ...
            uint256 transferAmount = (schemaBalances[uint256(_config.schema)] *
                _config.percentage) / MAX_PERCENT;
            ...
            if (transferAmount != 0) {
                ...
                IIncomeDestination(_config.destination).updateAssetsState(
                    comptroller,
                    asset
                );
        ...
        for (uint256 schemaValue = 0; schemaValue < totalSchemas; ) {
            newSchemaBalances[schemaValue] =
                ...
                totalTransferAmounts[schemaValue];
            assetsReserves[comptroller][asset][
                Schema(schemaValue)
            ] = newSchemaBalances[schemaValue];
    ...
    function _ensurePercentages() internal view {
        ...
        for (uint256 schemaValue = 0; schemaValue < totalSchemas; ) {
            if (
                totalPercentages[schemaValue] != MAX_PERCENT &&
                totalPercentages[schemaValue] != 0
    ...
    /**
     * @dev Returns the underlying asset address for the leToken
