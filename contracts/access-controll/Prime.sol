...
import {Scores} from "./Scores.sol";

import {IPrime} from "./IPrime.sol";
import {ILeToken} from "./interfaces/ILeToken.sol";
...
import {IPrimeLiquidityProvider} from "./IPrimeLiquidityProvider.sol";
    ...
    TimeManagerV8
{
    ...
    /// @notice Error thrown when invalid leToken is passed
    error InvalidLeToken();
    ...
    function getPendingRewards(
        address user
    ) external returns (PendingReward[] memory pendingRewards) {
     ...
     * @custom:error Throw InvalidLeToken if market is not valid
    ...
    function addMarket(
        ...
        if (
            ...
            PoolRegistryInterface(poolRegistry)
                .getPoolByComptroller(comptroller)
                .comptroller !=
            comptroller
        ...
        if (!isMarketExist) revert InvalidLeToken();
        ...
        if (leTokenForAsset[underlying] != address(0))
            revert AssetAlreadyExists();
        leTokenForAsset[underlying] = market;
     ...
     */
    function setLimit(
        uint256 _irrevocableLimit,
        uint256 _revocableLimit
    ) external {
        _checkAccessAllowed("setLimit(uint256,uint256)");
        if (
            _irrevocableLimit < totalIrrevocable ||
            _revocableLimit < totalRevocable
        ) revert InvalidLimit();
     ...
     */
    function accrueInterestAndUpdateScore(
        address user,
        address market
    ) external {
        _executeBoost(user, market);
    ...
    function claim() external {
        ...
        if (block.timestamp - userStakedAt < STAKING_PERIOD)
            revert WaitMoreTime();
    ...
    function claimInterest(
        ...
        return _claimInterest(leToken, msg.sender);
    }
    ...
    function claimInterest(
        ...
        return _claimInterest(leToken, user);
    }
    ...
    function calculateAPR(
        ...
        ILeToken leToken = ILeToken(market);
        uint256 borrow = leToken.borrowBalanceStored(user);
        uint256 exchangeRate = leToken.exchangeRateStored();
        uint256 balanceOfAccount = leToken.balanceOf(user);
        uint256 supply = (exchangeRate * balanceOfAccount) / EXP_SCALE;

        aprInfo.userScore = interests[market][user].score;
        aprInfo.totalScore = markets[market].sumOfMembersScore;

        aprInfo.xvsBalanceForScore = _xvsBalanceForScore(
            _xvsBalanceOfUser(user)
        );
        Capital memory capital = _capitalForScore(
    ...
    function estimateAPR(
        ...
        uint256 decimals = IERC20MetadataUpgradeable(_getUnderlying(market))
            .decimals();
     ...
     * @custom:error Throw MarketNotSupported if market is not supported
     */
    function accrueInterest(address leToken) public {
        Market storage market = markets[leToken];

        if (!market.exists) revert MarketNotSupported();

        address underlying = _getUnderlying(leToken);

        IPrimeLiquidityProvider _primeLiquidityProvider = IPrimeLiquidityProvider(
                primeLiquidityProvider
            );
        _primeLiquidityProvider.accrueTokens(underlying);
        uint256 totalAccruedInPLP = _primeLiquidityProvider.tokenAmountAccrued(
            underlying
        );
    ...
    function getInterestAccrued(
        ...
        accrueInterest(leToken);

        return _interestAccrued(leToken, user);
    }
    ...
    function _initializeMarkets(address account) internal {
        ...
        for (uint256 i; i < marketsLength; ) {
            ...
            interests[market][account].rewardIndex = markets[market]
                .rewardIndex;

            bytes[] memory priceUpdateData = new bytes[](0);
            uint256 score = _calculateScore(market, account, priceUpdateData);
    ...
    function _calculateScore(
        ...
        uint256 xvsBalanceForScore = _xvsBalanceForScore(
            _xvsBalanceOfUser(user)
        );
        ...
        oracle.updateAssetPrice(xvsToken, priceUpdateData);
        oracle.updatePrice(market, priceUpdateData);
     ...
     * @param leToken the market for which to claim
     * @param user the account for which to get the accrued interest
     * @return amount the amount of tokens transferred to the user
    ...
    function _claimInterest(
        ...
        uint256 amount = getInterestAccrued(leToken, user);
        amount += interests[leToken][user].accrued;

        interests[leToken][user].rewardIndex = markets[leToken].rewardIndex;
        delete interests[leToken][user].accrued;
        ...
        if (amount > asset.balanceOf(address(this))) {
            ...
            IPrimeLiquidityProvider(primeLiquidityProvider).releaseFunds(
                address(asset)
            );
        }
        ...
        emit InterestClaimed(user, leToken, amount);
    ...
    function _mint(bool isIrrevocable, address user) internal {
        ...
        if (
            totalIrrevocable > irrevocableLimit ||
            totalRevocable > revocableLimit
        ) revert InvalidLimit();
     ...
     * @param leToken the market for which we need to accrue rewards
     */
    function _executeBoost(address user, address leToken) internal {
        if (!markets[leToken].exists || !tokens[user].exists) {
            return;
        }

        accrueInterest(leToken);
        interests[leToken][user].accrued += _interestAccrued(leToken, user);
        interests[leToken][user].rewardIndex = markets[leToken].rewardIndex;
    }
    ...
    function _updateScore(address user, address market) internal {
        ...
        bytes[] memory priceUpdateData = new bytes[](0);
        uint256 score = _calculateScore(market, user, priceUpdateData);
        _market.sumOfMembersScore =
            _market.sumOfMembersScore -
            interests[market][user].score +
            score;
    ...
    function _xvsBalanceOfUser(address user) internal view returns (uint256) {
        (uint256 xvs, , uint256 pendingWithdrawals) = IXVSVault(xvsVault)
            .getUserInfo(xvsVaultRewardToken, xvsVaultPoolId, user);
     ...
     * @param market the market leToken address
    ...
    function _capitalForScore(
        ...
        capital.borrowCapUSD =
            (xvsPrice *
                ((xvs * markets[market].borrowMultiplier) / EXP_SCALE)) /
            EXP_SCALE;
        capital.supplyCapUSD =
            (xvsPrice *
                ((xvs * markets[market].supplyMultiplier) / EXP_SCALE)) /
            EXP_SCALE;
        ...
        if (supplyUSD >= capital.supplyCapUSD) {
            supply = supplyUSD != 0
                ? (supply * capital.supplyCapUSD) / supplyUSD
                : 0;
        }

        if (borrowUSD >= capital.borrowCapUSD) {
            borrow = borrowUSD != 0
                ? (borrow * capital.borrowCapUSD) / borrowUSD
                : 0;
        }
     ...
     * @param leToken the market for which to calculate the accrued interest
     * @param user the user for which to calculate the accrued interest
     * @return interestAccrued the number of underlying tokens accrued by the user since the last accrual
     */
    function _interestAccrued(
        address leToken,
        address user
    ) internal view returns (uint256) {
        Interest memory interest = interests[leToken][user];
        uint256 index = markets[leToken].rewardIndex - interest.rewardIndex;
    ...
    /**
     * @notice Returns the underlying token associated with the LeToken, or wrapped native token if the market is native market
     * @param leToken the market whose underlying token will be returned
     * @return underlying The address of the underlying token associated with the LeToken, or the address of the WRAPPED_NATIVE_TOKEN token if the market is NATIVE_MARKET
     */
    function _getUnderlying(address leToken) internal view returns (address) {
        if (leToken == NATIVE_MARKET) {
            return WRAPPED_NATIVE_TOKEN;
        }
        return ILeToken(leToken).underlying();
    }
     ...
     */
    function incomeDistributionYearly(
        address leToken
    ) public view returns (uint256 amount) {
        uint256 totalIncomePerBlockOrSecondFromPLP = IPrimeLiquidityProvider(
            primeLiquidityProvider
        ).getEffectiveDistributionSpeed(_getUnderlying(leToken));
     ...
     * @param leToken the market for which to fetch the APR
    ...
    function _calculateUserAPR(
        ...
        uint256 userYearlyIncome = (userScore *
            incomeDistributionYearly(leToken)) / totalScore;
        ...
        userSupplyIncomeYearly =
            (userYearlyIncome * totalCappedSupply) /
            totalCappedValue;
        userBorrowIncomeYearly =
            (userYearlyIncome * totalCappedBorrow) /
            totalCappedValue;
        supplyAPR = totalSupply == 0
            ? 0
            : ((userSupplyIncomeYearly * maximumBps) / totalSupply);
        borrowAPR = totalBorrow == 0
            ? 0
            : ((userBorrowIncomeYearly * maximumBps) / totalBorrow);
    }
}
