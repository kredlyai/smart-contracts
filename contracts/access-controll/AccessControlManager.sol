 ...
 * @author Kredly
 * @dev This contract is a wrapper of OpenZeppelin AccessControl extending it in a way to standartize access control within Kredly Smart Contract Ecosystem.
 * @notice Access control plays a crucial role in the Kredly governance model. It is used to restrict functions so that they can only be called from one
 ...
 * 
 * All restricted functions in Kredly Protocol use a hook to ACM in order to check if the caller has the right permission to call the guarded function.
...
```
    contract Comptroller is [...] AccessControlledV8 {
        [...]
        function setCollateralFactor(LeToken leToken, uint256 newCollateralFactorMantissa, uint256 newLiquidationThresholdMantissa) external {
