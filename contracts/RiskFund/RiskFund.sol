    ...
    IRiskFund
{
    ...
    event PoolRegistryUpdated(
        address indexed oldPoolRegistry,
        address indexed newPoolRegistry
    );

    /// @notice Emitted when shortfall contract address is updated
    event ShortfallContractUpdated(
        address indexed oldShortfallContract,
        address indexed newShortfallContract
    );

    /// @notice Emitted when convertible base asset is updated
    event ConvertibleBaseAssetUpdated(
        address indexed oldConvertibleBaseAsset,
        address indexed newConvertibleBaseAsset
    );

    /// @notice Emitted when PancakeSwap router contract address is updated
    event PancakeSwapRouterUpdated(
        address indexed oldPancakeSwapRouter,
        address indexed newPancakeSwapRouter
    );

    /// @notice Emitted when minimum amount to convert is updated
    event MinAmountToConvertUpdated(
        uint256 oldMinAmountToConvert,
        uint256 newMinAmountToConvert
    );

    /// @notice Emitted when pools assets are swapped
    event SwappedPoolsAssets(
        address[] markets,
    ...
    event TransferredReserveForAuction(
     ...
     */
    function setShortfallContractAddress(
        address shortfallContractAddress_
    ) external onlyOwner {
        ...
        emit ShortfallContractUpdated(
            oldShortfallContractAddress,
            shortfallContractAddress_
        );
    }
    ...
    function setMinAmountToConvert(uint256 minAmountToConvert_) external {
        ...
        minAmountToConvert = minAmountToConvert_;
        emit MinAmountToConvertUpdated(
            oldMinAmountToConvert,
            minAmountToConvert_
        );
    }
    ...
    function setConvertibleBaseAsset(address _convertibleBaseAsset) external {
        ...
        require(
            _convertibleBaseAsset != address(0),
            "Risk Fund: new convertible base asset address invalid"
        );

        address oldConvertibleBaseAsset = convertibleBaseAsset;
        convertibleBaseAsset = _convertibleBaseAsset;

        emit ConvertibleBaseAssetUpdated(
            oldConvertibleBaseAsset,
            _convertibleBaseAsset
        );
    }

    /**
     * @notice Swap array of pool assets into base asset's tokens of at least a minimum amount
     * @param markets Array of leTokens whose assets to swap for base asset
    ...
    function swapPoolsAssets(
        ...
        ensureNonzeroAddress(poolRegistry_);
        require(
            markets.length == amountsOutMin.length,
            "Risk fund: markets and amountsOutMin are unequal lengths"
        );
        require(
            markets.length == paths.length,
            "Risk fund: markets and paths are unequal lengths"
        );
        ...
        for (uint256 i; i < marketsCount; ++i) {
            address comptroller = address(LeToken(markets[i]).comptroller());

            PoolRegistry.KredlyPool memory pool = PoolRegistry(poolRegistry_)
                .getPoolByComptroller(comptroller);
            require(
                pool.comptroller == comptroller,
                "comptroller doesn't exist pool registry"
            );
            require(
                Comptroller(comptroller).isMarketListed(LeToken(markets[i])),
                "market is not listed"
            );

            uint256 swappedTokens = _swapAsset(
                LeToken(markets[i]),
                comptroller,
                amountsOutMin[i],
                paths[i]
    ...
    function transferReserveForAuction(
        ...
        require(
            msg.sender == shortfall_,
            "Risk fund: Only callable by Shortfall contract"
        );
        require(
        ...
        unchecked {
            assetsReserves[convertibleBaseAsset] =
                assetsReserves[convertibleBaseAsset] -
                amount;
        }

        emit TransferredReserveForAuction(comptroller, amount);
        IERC20Upgradeable(convertibleBaseAsset).safeTransfer(
            shortfall_,
            amount
        );
    ...
    function getPoolsBaseAssetReserves(
        ...
        require(
            ...
            "Risk Fund: Comptroller address invalid"
        );
    ...
    function _swapAsset(
        ...
        require(
            amountOutMin != 0,
            "RiskFund: amountOutMin must be greater than 0 to swap leToken"
        );
        uint256 totalAmount;

        address underlyingAsset = leToken.underlying();
        address convertibleBaseAsset_ = convertibleBaseAsset;
        uint256 balanceOfUnderlyingAsset = _poolsAssetsReserves[comptroller][
            underlyingAsset
        ];
        ...
        ResilientOracleInterface oracle = ComptrollerViewInterface(comptroller)
            .oracle();

        {
            bytes[] memory priceUpdateData = new bytes[](0);
            oracle.updateAssetPrice(convertibleBaseAsset_, priceUpdateData);
        }
        
        Exp memory baseAssetPrice = Exp({
            mantissa: oracle.getPrice(convertibleBaseAsset_)
        });
        uint256 amountOutMinInUsd = mul_ScalarTruncate(
            baseAssetPrice,
            amountOutMin
        );

        require(
            amountOutMinInUsd >= minAmountToConvert,
            "RiskFund: minAmountToConvert violated"
        );
        ...
        if (underlyingAsset != convertibleBaseAsset_) {
            require(
                ...
                "RiskFund: swap path must start with the underlying asset"
            );
            require(
            ...
            IERC20Upgradeable(underlyingAsset).approveOrRevert(
                pancakeSwapRouter_,
                0
            );
            IERC20Upgradeable(underlyingAsset).approveOrRevert(
            ...
            uint256[] memory amounts = IPancakeswapV2Router(pancakeSwapRouter_)
