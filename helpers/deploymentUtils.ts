...
import { getTokenConfig } from "./deploymentConfig";
import { Comptroller, MockToken } from "../typechain";
...
export const getUnderlyingToken = async (assetSymbol: string, tokensConfig: TokenConfig[]) => {
...
export const getUnregisteredPools = async (poolConfig: PoolConfig[], hre: HardhatRuntimeEnvironment,): Promise<PoolConfig[]> => {
  const { deployments } = hre;
...
export const getUnregisteredLeTokens = async (poolConfig: PoolConfig[], hre: HardhatRuntimeEnvironment): Promise<PoolConfig[]> => {
  const { deployments } = hre;
  const registry = await ethers.getContract("PoolRegistry");
  ...
  return Promise.all(
    poolConfig.map(async (pool: PoolConfig) => {
      const isRegistered = await Promise.all(
        pool.letokens.map(async (leTokenConfig: LeTokenConfig) => {
          const leToken = await deployments.getOrNull(`LeToken_${leTokenConfig.name}`);
          if (!leToken) {
            // If the LeToken deployment doesn't exist, it's not registered
            return false;
          }
          return registeredLeTokens.includes(leToken.address);
        }),
      );
      return { ...pool, letokens: pool.letokens.filter((_, idx: number) => !isRegistered[idx]) };
    }),
  )
}

export const getUnregisteredRewardsDistributors = async (poolConfig: PoolConfig[], hre: HardhatRuntimeEnvironment): Promise<PoolConfig[]> => {
  const { deployments } = hre;
  const registry = await ethers.getContract("PoolRegistry");
  const registeredPools = await registry.getAllPools();
  const comptrollers = await Promise.all(
